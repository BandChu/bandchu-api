...
import com.bandchu.api.domain.chat.controller.ChatMessageController  // ✅
...
class ChatMessageControllerTest :
        FunSpec({
            ...
            test("메시지 전송 성공 시 201 CREATED와 메시지 정보 반환") {
                ...
                // 서비스 메서드 호출 시 동작 정의
                every { chatMessageService.sendMessage(roomId, 1L, request) } returns
                        expectedResponse
                ...
                verify(exactly = 1) { chatMessageService.sendMessage(roomId, 1L, request) }
            }
            ...
            test("파일 메시지 전송 시 fileUrl이 포함되어야 함") {
                ...
                mockMvc.perform(
                        ...
                        .andExpect(
                                jsonPath("$.data.fileUrl").value("https://example.com/image.jpg")
                        )
